...
const searchCourse = async (req, res) => {
  ...
  try {
    const course = await Course.find()
      .or([{ Title: Title }, { Subject: Subject }])
...
const editBiography = async (req, res) => {
  try {
    ...
    const id = req.user;
    const updated = await Instructor.findByIdAndUpdate(id, {
      Biography: Biography,
    });
    if (!updated) return res.status(400).json({ error: "No user found" });
...
const defineDiscount = async (req, res) => {
  ...
  try {
    ...
    const course = await Course.findByIdAndUpdate(
      CourseID,
      { $set: { Discount: NewDiscount, DiscountEndDate: EndDate } },
      { new: true }
    );
...
const createQuestion = async (req, res) => {
  const { QuestionHeader, Answer1, Answer2, Answer3, Answer4, correctAnswer } =
    req.body;
  const examID = await Exam.find({}, { _id: 1 })
    .sort({ _id: -1 })
    .limit(1)
    .select("_id");
  ...
  try {
    ...
    const qid = await Question.find({}, { _id: 1 })
      .sort({ _id: -1 })
      .limit(1)
      .select("_id");
    const examm = await Exam.updateOne(
      { _id: examID },
      { $push: { Questions: qid } }
    );
...
const createExam = async (req, res) => {
  ...
  try {
    ...
    const eid = await Exam.find({}, { _id: 1 })
      .sort({ _id: -1 })
      .limit(1)
      .select("_id");
    console.log(eid);
    const examm = await Subtitle.updateOne(
      { _id: subtitleId },
      { $push: { Exercises: eid } }
    );
...
const EditInstructorinfo = async (req, res) => {
  ...
  try {
    const instructor = await Instructor.findByIdAndUpdate(req.user, {
      Firstname,
      Lastname,
      Gender,
    });
